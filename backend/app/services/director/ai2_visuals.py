...
def _image_prompt(scene: dict[str, Any], style: str, language: str) -> str:
    ...
    # Stil eşleme
    style_map = {
        "cinematic": "cinematic, anamorphic, film grain, 24fps look, rich shadows, deep blacks, warm highlights",
        "punchy": "high contrast, vibrant colors, sharp focus, neon accents, clean lines, bold shadows",
        "minimal": "clean composition, soft tones, negative space, pastel palette, diffused lighting, zen",
        "egitici": "bright clear lighting, shallow depth of field, sharp details, natural colors, professional clean",
        "eglenceli": "playful lighting, saturated colors, dynamic angles, whimsical, fun mood, vibrant",
        "hikaye": "dramatic chiaroscuro, moody shadows, warm amber tones, textured, filmic depth",
        "yemek": "food photography style, golden lighting, warm tones, shallow DOF, steam/movement visible",
        "teknoloji": "clean futuristic, cool blue/cyan tones, hard light, metallic reflections, crisp",
        "dogal": "natural lighting, earth tones, soft shadows, organic texture, realistic skin tones",
        "vintage": "warm faded tones, film grain, light leaks, muted colors, retro composition",
    }
    style_desc = style_map.get(style.lower().strip(), "cinematic")

    return (
        f"Vertical 9:16 cinematic still for YouTube Shorts. "
        f"Role: '{role}'. Style: {style} — {style_desc}. "
        f"Shot description: {visual}. "
        f"Sahnenin ruhu: '{narration[:100]}'. "
        f"Mood lighting, professional color grade, no watermark, no UI chrome, no text overlay. "
        f"IMPORTANT: Do NOT render text, logos, or UI elements. "
        f"Photorealistic, high detail, 8K quality, cinematic depth of field. "
